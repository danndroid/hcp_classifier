...
def compute_egde_list(causal_graph):

    edge_list = np.transpose((causal_graph!=0).nonzero())
    nx_edge_list = [(e[0],e[1]) for e in edge_list]

    return edge_list, nx_edge_list
    

def compute_graph_measures(nx_edge_list):
    G = nx.DiGraph()
...
def compute_edge_features(fc, nx_edge_list):
    #G = nx.from_numpy_array(fc)
    #edge_list = np.array(G.edges)
    i_coords, j_coords = zip(*nx_edge_list)
...
def create_graph_dataset(causal_fcs, sparse_fcs, root, labels, scale_features=False):
    ...
    for idx, causal_fc, fc in zip(np.arange(N), causal_fcs, sparse_fcs):
        #print(idx)

        #G = nx.DiGraph()
        edge_list, nx_edge_list = compute_egde_list(causal_fc)
        #G.add_edges_from(nx_edge_list)

        try:
            graph_measures = compute_graph_measures(nx_edge_list)
        except:
            print(f'Graph {idx}')
            graph_measures = np.zeros((100,3))

        edge_features = compute_edge_features(fc, nx_edge_list)
...
class HCPCausalDataset(Dataset):
